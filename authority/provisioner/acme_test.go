...
func TestACME_GetAttestationIntermediates(t *testing.T) {
	appleCA, err := os.ReadFile("testdata/certs/apple-att-ca.crt")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			p := &ACME{
				Type:                     tt.fields.Type,
				Name:                     tt.fields.Name,
				AttestationIntermediates: tt.fields.AttestationIntermediates,
			}
			if err := p.Init(Config{
				Claims:    globalProvisionerClaims,
				Audiences: testAudiences,
			...
			got, got1 := p.GetAttestationIntermediates()
			if tt.want == nil && got != nil {
				t.Errorf("ACME.GetAttestationIntermediates() got = %v, want %v", got, tt.want)
			} else if !tt.want.Equal(got) {
				t.Errorf("ACME.GetAttestationIntermediates() got = %v, want %v", got, tt.want)
			}
			if got1 != tt.want1 {
				t.Errorf("ACME.GetAttestationIntermediates() got1 = %v, want %v", got1, tt.want1)
			}
		})
	}
}
