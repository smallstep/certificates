...
type challengeValidationController struct {
	...
	webhooks            []*Webhook
	challengeValidators []stepapi.ChallengeValidator
}
...
func newChallengeValidationController(client *http.Client, tw httptransport.Wrapper, webhooks []*Webhook, challengeValidators []stepapi.ChallengeValidator) *challengeValidationController {
	scepHooks := []*Webhook{}
...
func (c *challengeValidationController) ValidateCustom(ctx context.Context, csr *x509.CertificateRequest) error {
	for _, validator := range c.challengeValidators {
		if err := validator.Validate(ctx, stepapi.ChallengeValidatorRequest{csr: csr}); err != nil {
