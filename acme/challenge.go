...
func doTPMAttestationFormat(_ context.Context, prov Provisioner, ch *Challenge, jwk *jose.JSONWebKey, att *attestationObject) (*tpmAttestationData, error) {
	...
	intermediates, ok := prov.GetAttestationIntermediates()
	if !ok {
...
func doAppleAttestationFormat(_ context.Context, prov Provisioner, _ *Challenge, att *attestationObject) (*appleAttestationData, error) {
	...
	// Use configured intermediates certificates if configured.
	intermediates, ok := prov.GetAttestationIntermediates()
	if !ok {
		intermediates = x509.NewCertPool()
	}
	...
	// Add intermediates from the x5c
	for _, v := range x5c[1:] {
		der, ok = v.([]byte)
		if !ok {
			return nil, NewDetailedError(ErrorBadAttestationStatementType, "x5c is malformed")
...
// Yubico Attestation Intermediate A 1 (signed by Yubico Attestation Root 1)
// Yubico Attestation Intermediate B 1 (signed by Yubico Attestation Root 1)
// Yubico PIV Attestation A 1 (signed by Yubico Attestation Intermediate A 1)
// Yubico PIV Attestation B 1 (signed by Yubico Attestation Intermediate B 1)
// https://developers.yubico.com/PKI/yubico-intermediate.pem
const yubicoPIVIntermediates = `-----BEGIN CERTIFICATE-----
...
func doStepAttestationFormat(_ context.Context, prov Provisioner, ch *Challenge, jwk *jose.JSONWebKey, att *attestationObject) (*stepAttestationData, error) {
	...
	// Use configured or default attestation intermediates if non is configured
	intermediates, ok := prov.GetAttestationIntermediates()
	if !ok {
		certs, err := pemutil.ParseCertificateBundle([]byte(yubicoPIVIntermediates))
		if err != nil {
			return nil, WrapErrorISE(err, "error parsing intemediate bundle")
		}
		intermediates = x509.NewCertPool()
	...
	// Append intermediates from the x5c
	for _, v := range x5c[1:] {
		der, ok = v.([]byte)
		if !ok {
			return nil, NewDetailedError(ErrorBadAttestationStatementType, "x5c is malformed")
