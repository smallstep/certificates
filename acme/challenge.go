...
func (hc *http01Challenge) validate(db nosql.DB, jwk *jose.JSONWebKey, vo validateOptions) (challenge, error) {
	...
	if err != nil {
		if err = hc.iterateRetry(db, ConnectionErr(errors.Wrapf(err, "error doing http GET for url %s", url))); err != nil {
			return nil, err
		}
		return hc, nil
	}
	if resp.StatusCode >= 400 {
		if err = hc.iterateRetry(db, ConnectionErr(errors.Errorf("error doing http GET for url %s with status code %d",
...
func (tc *tlsALPN01Challenge) validate(db nosql.DB, jwk *jose.JSONWebKey, vo validateOptions) (challenge, error) {
	...
	if err != nil {
		if err = tc.iterateRetry(db, ConnectionErr(errors.Wrapf(err, "error doing TLS dial for %s", hostPort))); err != nil {
			return nil, err
		}
		return tc, nil
	}
	...
	if len(certs) == 0 {
		if err = tc.iterateRetry(db, RejectedIdentifierErr(errors.Errorf("%s challenge for %s resulted in no certificates",
...
func (dc *dns01Challenge) validate(db nosql.DB, jwk *jose.JSONWebKey, vo validateOptions) (challenge, error) {
	...
	if err != nil {
		if err = dc.iterateRetry(db, DNSErr(errors.Wrapf(err, "error looking up TXT "+
	...
	if !found {
		if err = dc.iterateRetry(db, RejectedIdentifierErr(errors.Errorf("keyAuthorization "+
