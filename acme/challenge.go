...
func (hc *http01Challenge) validate(db nosql.DB, jwk *jose.JSONWebKey, vo validateOptions) (challenge, error) {
	...
	if err != nil {
		if err = hc.updateRetry(db, ConnectionErr(errors.Wrapf(err, "error doing http GET for url %s", url))); err != nil {
			return nil, err
		}
		return hc, nil
	}
	if resp.StatusCode >= 400 {
		if err = hc.updateRetry(db, ConnectionErr(errors.Errorf("error doing http GET for url %s with status code %d",
	...
	if keyAuth != expected {
		if err = hc.updateRetry(db, RejectedIdentifierErr(errors.Errorf("keyAuthorization does not match; "+
...
func (tc *tlsALPN01Challenge) validate(db nosql.DB, jwk *jose.JSONWebKey, vo validateOptions) (challenge, error) {
	// If already valid or invalid then return without performing validation.
	if tc.getStatus() == StatusValid {
		return tc, nil
	}
	if tc.getStatus() == StatusInvalid {
		// TODO: Resolve segfault on upd.save
		upd := tc.clone()
		upd.Status = StatusPending
	...
	if err != nil {
		if err = tc.updateRetry(db, ConnectionErr(errors.Wrapf(err, "error doing TLS dial for %s", hostPort))); err != nil {
			return nil, err
		}
		return tc, nil
	}
	...
	if len(certs) == 0 {
		if err = tc.updateRetry(db, RejectedIdentifierErr(errors.Errorf("%s challenge for %s resulted in no certificates",
	...
	if !cs.NegotiatedProtocolIsMutual || cs.NegotiatedProtocol != "acme-tls/1" {
		if err = tc.updateRetry(db, RejectedIdentifierErr(errors.Errorf("cannot negotiate ALPN acme-tls/1 protocol for "+
	...
	if len(leafCert.DNSNames) != 1 || !strings.EqualFold(leafCert.DNSNames[0], tc.Value) {
		if err = tc.updateRetry(db, RejectedIdentifierErr(errors.Errorf("incorrect certificate for tls-alpn-01 challenge: "+
...
func (dc *dns01Challenge) validate(db nosql.DB, jwk *jose.JSONWebKey, vo validateOptions) (challenge, error) {
	...
	if err != nil {
		if err = dc.updateRetry(db, DNSErr(errors.Wrapf(err, "error looking up TXT "+
	...
	if !found {
		if err = dc.updateRetry(db, RejectedIdentifierErr(errors.Errorf("keyAuthorization "+
